...
// ExecuteScriptContent uploads and runs script content (not a file) on remote server
func (c *Client) ExecuteScriptContent(scriptContent string, args ...string) (string, error) {
	client, err := c.Connect()
	if err != nil {
		return "", err
	}
	...
	// Generate remote path
	remoteScriptPath := fmt.Sprintf("/tmp/scaprun_script_%d.sh", time.Now().Unix())
